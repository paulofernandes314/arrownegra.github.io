...
def show_save_data_settings():
    class FirstRun(xbmcgui.WindowXMLDialog):
        ...
        def onInit(self):
            ...
            self.thumbs = 313
            self.show_dialog()
            self.controllist = [self.trakt, self.debrid, self.login,                        self.advanced, self.favourites, self.whitelist,                        self.cache, self.packages, self.thumbs]
            self.controlsettings = ['keeptrakt', 'keepdebrid', 'keeplogin',                        'keepadvanced', 'keepfavourites',                        'keepwhitelist', 'clearcache', 'clearpackages',                        'clearthumbs']
