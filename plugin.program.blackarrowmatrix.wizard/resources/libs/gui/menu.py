    ...
    for trakt in traktit.ORDER:
        if xbmc.getCondVisibility('System.HasAddon({0})'.format(traktit.TRAKTID[trakt]['plugin'])):
            ...
            if not os.path.exists(path):
                ...
                directory.add_file('[COLOR red]Addon Data: Click Here To Authorize[/COLOR]', {'mode': 'authtrakt', 'name': trakt}, icon=icon, fanart=fanart, menu=menu)
            else:
    ...
    for debrid in debridit.ORDER:
        if xbmc.getCondVisibility('System.HasAddon({0})'.format(debridit.DEBRIDID[debrid]['plugin'])):
            ...
            if not os.path.exists(path):
                ...
                directory.add_file('[COLOR red]Addon Data: Click Here To Authorize[/COLOR]', {'mode': 'authdebrid', 'name': debrid}, icon=icon, fanart=fanart, menu=menu)
            else:
    ...
    for login in loginit.ORDER:
        if xbmc.getCondVisibility('System.HasAddon({0})'.format(loginit.LOGINID[login]['plugin'])):
            ...
            if not os.path.exists(path):
                ...
                directory.add_file('[COLOR red]Addon Data: Click Here To Authorize[/COLOR]', {'mode': 'authlogin', 'name': login}, icon=icon, fanart=fanart, menu=menu)
            else:
